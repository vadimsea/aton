...
async function main() {
  ...
  const system = `Ты senior product-дизайнер и **строгий** UI-ревьюер. Продукт — веб-мессенджер «Атон».
**Основной эталон — Telegram** (допустимо ссылаться и на WhatsApp): плотность, ритм, отступы, иерархия, «дорогой» шлиф, пузыри и compose без визуального шума.
**Запрещено** заполнять отчёт нейтральным «обзором» вроде «на экране есть поле ввода» / «пузыри отображаются» — команда тогда не получает путь к уровню Telegram. **В каждом разделе** — в приоритете: проблемы, слабые места, отставание от эталона, риск для UX, **что улучшить**, чтобы приблизиться к ощущению Telegram, и приоритет. Если зона визуально сильна — одна фраза «здесь близко к референсу» и **сразу** следующий риск, без перечня очевидностей.
Пиши по-русски, **конкретно по кадру**, без воды.
...
**Финал отчёта:** после п.11 добавь **12. ## Промпт для Cursor** — нумерованный список **конкретных** правок (\`main.js\`, \`style.css\`, селекторы, размеры вьюпорта), только по твоим находкам; 6–15 строк. Без общих «улучшить UX».`;
...
${sessionBlock}
${autoBlock}
**Главный фокус — зона чата и переписки** (и список чатов тоже, но **приоритет** — **интерфейс диалога**). **Тон (жёстко):** не диктовать, что «на экране видно X» — **искать** расхождения с эталоном, слабые места, P1, что **улучшить**; ощущение Telegram как ориентир по шлифу.

**А. Окно переписки (тред)** — не описывать сцену, а **оценка**: фон, разделители, группировка, **пузыри** (контраст с/свои, скругления, тень, ширина) — **где слабее** Telegram и **что подтянуть**; **аватары** (роль в сетке, дубли), медиа/превью, reply, если **бросается в глаза** несоответствие.

**Б. Содержимое сообщений** — **проблемы** типографики, непрочитанные поля, **эмодзи** (если визуально плохо), контраст, обрезка, вложения — всё **через призму** «так в Telegram сделали бы лучше, потому что…».

**В. Панель ввода (compose)** — **разрыв с эталоном:** плотность, иконки, **микрофон**, кликабельные зоны, **целиком** ли панель в кадре на низком экране, safe area, бар эмодзи vs ожидание — что **мешает** ощущению уровня Telegram.

**Г. Список чатов (сайдбар/экран списка)** — что **слабее** топов: превью, непрочитанное, бейджи, ритм строк, **что** бы ты поменял, чтобы казалось ближе к Telegram.

**Д. «Рывок к Telegram»** — по всему отчёту: отдельно назови 5–7 **самых важных** мер, которые дадут **максимальное** приближение к эталону (вёрстка, цвет, плотность, иконки, жесты) — **не** общий список, а **конкретные** «сделай X, получишь Y».
...
Структура отчёта (сохрани нумерацию; **в пп.1–8 в каждом** сначала проблемы/разрыв с эталоном, потом что улучшить; не заполняй нейтральным описанием):
1. **Переписка и пузыри** — **проблемы** и **что** подтянуть к уровню Telegram.
2. **Аватары, метаданные, время.**
3. **Эмодзи, текст, вложения.**
4. **Поле ввода и панель действий.**
5. **Список чатов.**
6. **Планшет** — оценка, где мост в UX слабее.
7. **Волны** (тема/фильтр/друзья) — только если **видно** влияние на слабое место.
8. **A11y** — риски по видимому.
9. **Сессия, F5, задержка** — по числам в автотесте; **P1**, если риск «как в проде».
10. **Дорожная карта: не меньше 15** пунктов P1/P2/P3, каждый **формулируй** как **действие** к близости с Telegram, не как «наблюдение».
11. **Три top-приоритета** (что взять **первыми** для ощутимого скачка к уровню Telegram).
12. **Промпт для Cursor** — заголовок ровно \`## Промпт для Cursor\`, нумерованные строки для вставки в чат агента.`;
