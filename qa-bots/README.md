...
|--------|------------------|------|
| `frontend-daily.mjs` | 05:00 | Playwright: **замер сессии** (F5+токен, повторный F5, `networkidle`) → затем волны **десктоп / планшет / мобилка**, чат, скрины → **Groq vision** (числа сессии в промпте и в .md, без опоры только на картинки). |
