...
| | Формат | Где |
|---|--------|-----|
| **Фронт** | `frontend-YYYY-MM-DD.md` — Markdown | текст отчёта, удобно вставлять в чат |
...
| **Бэк** | `backend-YYYY-MM-DD.md` / `.html` | API, сеть, рекомендации (без картинок) |
...
В текстовом анализе боты по возможности заканчивают разделом **«Промпт для Cursor»** — конкретные задачи по фактам прогона (без общих «улучшить UX»). Соц-бот (**только API**) не советует интерфейс «как Telegram», если журнал — только HTTP.
