...
2. **Связь API → UX/клиент** — как ответы/ошибки/задержки бьют по опыту (в т.ч. **чат, лента сообщений, вложения** в духе **Telegram/WhatsApp**); кратко по данным.
3. **По логу смоука** — риски.
4. **Производительность** — задержки, что мерить в проде.
5. **HTTP/API** — ошибки, 404, rate limit (обобщённо).
6. **Безопасность** — токены, CORS, заголовки, тексты ошибок.
7. **Надёжность** — ретраи, cold start (Render) если уместно.
8. **Наблюдаемость** — health, логи.
9. **Улучшения** (P1/P2/P3), отдельно отметь пункты, влияющие на **UX** в клиенте.
