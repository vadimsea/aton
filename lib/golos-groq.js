 ...
 */
async function fetchGolosReply({ history, username, displayName, fromVoice = false } = {}) {
  ...
  if (!turnList.length || turnList[turnList.length - 1].role !== "user") {
    return "Внутренняя ошибка контекста. Напишите ещё раз.";
  }
  ...
  const system = [
    ...
    "Веди беседу как в чате с ChatGPT: помни, о чём вы уже говорили в этой ветке, развивай тему, связывай ответы с предыдущими репликами.",
    "Собеседник один — пиши на «ты» по смыслу, но без фамильярности, если уместен нейтральный тон, используй его.",
    `Собеседник: @${username} (отображаемое имя: ${label}).`,
    "Отвечай по существу, на русском, спокойно и дружелюбно. Ты цифровой помощник, не выдавай себя за человека.",
    "Если вопрос вне твоей компетенции, скажи честно и мягко предложи написать в поддержку или к людям.",
    fromVoice
      ? "Последнее сообщение пришло голосом: ответь сравнительно коротко, 1–4 предложения, удобно для прослушивания, но с учётом контекста выше."
      : null,
  ]
    .filter(Boolean)
    .join(" ");
...
/** Озвучка ответа: OpenAI TTS при наличии ключа, иначе Google TTS (ограничение ~200 символов на сегмент). */
async function synthesizeGolosReplyToDataUrl(fullText) {
  ...
  try {
    ...
    const b64 = await getAudioBase64(short, { lang: "ru", slow: false, timeout: 20000 });
    if (b64) {
