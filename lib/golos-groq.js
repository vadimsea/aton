...
async function fetchGolosReply({ history, fromVoice = false } = {}) {
  ...
  let res = await groqChat(model);
  if (!res.ok) {
    const t = await res.text();
    console.error("Groq API error:", res.status, model, t);
    ...
    if (canRetryFallback) {
      ...
      res = await groqChat(fallbackModel);
    } else {
  ...
  if (!res.ok) {
    ...
    console.error("Groq API error (после fallback):", res.status, fallbackModel, t2);
